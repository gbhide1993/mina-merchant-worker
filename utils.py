...
def upload_twilio_media_to_gcs(media_url: str, content_type: str, phone: str) -> str:
    """
    Downloads media from Twilio and uploads it to GCS.
    Returns gs:// path.
    """

    if not all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, GCS_BUCKET]):
        raise RuntimeError("Twilio or GCS env vars missing")

    # 1️⃣ Download from Twilio
    resp = requests.get(
        media_url,
    ...
    object_name = f"merchant_audio/{safe_phone}/{timestamp}.{ext}"
...
def transcribe_file_multilang(file_path: str) -> str:
    ...
    Optimized for Indian merchant speech (Hindi / Marathi / Hinglish).
    """
